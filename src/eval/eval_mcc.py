...
# Paths are derived from this file's location (src/eval/eval_mcc.py ->
# project_root/data/...) instead of being hardcoded to one developer's
# machine. This is the only thing that makes the script runnable in CI,
# on a teammate's laptop, or inside the Docker image.
_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
PRED_PATH = str(_PROJECT_ROOT / "data" / "preds.json")
GOLD_PATH = str(_PROJECT_ROOT / "data" / "gold.json")
    ...
    for p in preds:
        ...
        if cid in gold_map:
            if p["verdict"] == "unknown":
                # An "unknown" verdict (LLM call failed) is not a model
                # prediction — including it as a binary 0/1 would silently
                # count LLM outages as correct or incorrect guesses. Skip
                # and report it separately instead.
                skipped.append(cid)
                continue
...
def main():
    ...
    y_true, y_pred, skipped = load_labels(args.preds, args.gold)

    if skipped:
        print(f"WARNING: {len(skipped)} chunk(s) had verdict='unknown' "
              f"(LLM call failure) and were excluded from scoring: {skipped}")

    if len(y_true) == 0:
        print("No scoreable predictions (after excluding 'unknown' verdicts). "
    ...
    print(f"Scored {len(y_true)} of {len(y_true) + len(skipped)} chunks "
          f"({len(skipped)} skipped due to 'unknown' verdict)")
    print("Accuracy:", round(float(acc), 4))

    # Precision/recall/F1, in addition to MCC. In a compliance tool, a false
    # negative (predicting "pass" when the document actually violates the
    # regulation) is a worse failure than a false positive, so recall on
    # the "fail" (positive) class deserves explicit reporting rather than
    # being folded into a single aggregate score.
    precision = precision_score(y_true, y_pred, pos_label=1, zero_division=0)
    recall = recall_score(y_true, y_pred, pos_label=1, zero_division=0)
    f1 = f1_score(y_true, y_pred, pos_label=1, zero_division=0)
    print(f"Precision (fail-class): {precision:.4f}")
    print(f"Recall    (fail-class): {recall:.4f}")
    print(f"F1        (fail-class): {f1:.4f}")

    # Robust MCC
    mcc = safe_mcc(y_true, y_pred, policy=args.policy)
    print("MCC (policy=%s):" % args.policy, round(float(mcc), 4) if mcc == mcc else mcc)  # mcc==mcc is False for NaN
