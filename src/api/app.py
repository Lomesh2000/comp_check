...
# NOTE: build_fused_subgraph's signature now also requires the static KG's
# node order + embeddings + the SentenceTransformer model instance (used to
# re-embed eventic hit strings at query time and match them against the
# static graph). These are loaded once in init_global_state() below.
...
STATIC_NODE_EMB_PATH = DATA_DIR / "static" / "node_embeddings.npz"
...
def run_compliance_text(
    ...
    Gfus, hits, P, N, fallback_used = build_fused_subgraph(
        text,
        text_vec,
        _eventic_nodes,
        _eventic_embs,
    ...
    if reply:
        # NOTE: Tempt3 (the active prompt template) asks the LLM to answer
        # with a single word, "PASS" or "FAIL". The legacy
        # "<Compliance Check Passed/Failed>" tag format is no longer what
        # any prompt variant produces — checked first only as a defensive
        ...
        if "<Compliance Check Passed>" in reply:
            ...
            if lower.startswith("fail"):
                ...
                # Genuinely ambiguous reply (neither PASS/FAIL nor a
                # violation keyword). Defaulting silently to "pass" here
                # would hide a parsing failure as a compliance pass — the
                # worst possible failure mode for this tool. Surface it as
                # "unknown" instead and keep the raw reply for review.
                verdict = "unknown"
...
def init_global_state():
    ...
    global _static_node_order, _static_embs
    ...
    # Load precomputed static KG node embeddings (required by the current
    # build_fused_subgraph(), which matches eventic hits against the static
    # graph via embedding similarity rather than exact string match).
    # Degrade to an empty static graph if missing, rather than crashing —
    # this keeps /health reporting "degraded" instead of the process dying.
    try:
        if STATIC_NODE_EMB_PATH.exists():
            _static_node_order, _static_embs = load_node_embeddings(str(STATIC_NODE_EMB_PATH))
            logger.info(f"Loaded static node embeddings: {len(_static_node_order)} nodes")
        else:
            logger.warning(f"Static node embeddings file not found: {STATIC_NODE_EMB_PATH}")
            _static_node_order, _static_embs = [], None
    except Exception as e:
        logger.error(f"Failed to load static node embeddings: {e}")
        _static_node_order, _static_embs = [], None
...
async def health_check():
    ...
    return HealthCheckResponse(
        ...
        loaded_models={
            ...
            "static_node_embeddings": _static_embs is not None,
        },
    )
