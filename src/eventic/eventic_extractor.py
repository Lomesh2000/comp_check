...
if __name__ == "__main__":
    ...
    # Output path is fixed and matches what app.py / rag_runner.py load
    # (EVENTIC_GRAPH_PATH = data/eventic_graph.gpickle). Writing per-file
    # outputs with relative paths previously scattered *_eventic_graph.gpickle
    # files into whatever directory the script happened to be run from,
    # and none of them were the file the rest of the pipeline actually reads.
    OUT_PATH = os.path.join(data_dir, "eventic_graph.gpickle")

    # Only PDFs/text files in data/ itself are source regulatory documents.
    # Everything else in data/ (chunks.json, gold.json, preds.json,
    # eval.json, *.gpickle, *.npz, the business/ and static/ and queries/
    # subdirectories) is pipeline output/config, not a document to mine
    # for deontic triples. Previously the script globbed every entry in
    # data/ with no extension filter, which fed JSON and even a binary
    # .gpickle file through the text extractor and polluted the graph
    # with garbage triples parsed out of pickle byte sequences.
    SOURCE_EXTENSIONS = (".pdf", ".txt", ".md", ".text")
    EXCLUDED_NAMES = set()  # extend here if you add more non-source files at the top level
    ...
    for fname in sorted(os.listdir(data_dir)):
        ...
        if not lower.endswith(SOURCE_EXTENSIONS):
            print(f"[SKIP] Not a source document, skipping: {fname}")
            continue
    ...
    if not all_texts:
        raise RuntimeError(
            f"No readable source documents (.pdf/.txt/.md) found directly in {data_dir}. "
            "Put your regulatory text file(s) there."
        )

    # Merge triples from all source documents into a single combined graph,
    # then write it once to the fixed path everything downstream expects.
    combined_graph = nx.DiGraph()
    total_triples = 0
    for fname, doc_text in all_texts:
        print(f"\n=== Processing document: {fname} ===")
        triples = extract_eventic_triples(doc_text)
        print(f"Extracted {len(triples)} triples from {fname}")
        total_triples += len(triples)
        doc_graph = build_eventic_graph(triples)
        combined_graph = nx.compose(combined_graph, doc_graph)

    print(f"\nCombined eventic graph: {combined_graph.number_of_nodes()} nodes, "
          f"{combined_graph.number_of_edges()} edges (from {total_triples} total triples "
          f"across {len(all_texts)} document(s))")

    with open(OUT_PATH, "wb") as f:
        pickle.dump(combined_graph, f)
    print(f"Saved eventic graph to {OUT_PATH}")