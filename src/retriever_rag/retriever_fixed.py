#!/usr/bin/env python3
"""
retriever.py — FIXED for mixed corpus (filename vs source_doc fields).

What changed:
  - All references to c["filename"] now use _get_doc_name(c) helper
  - This handles both old chunks (field "filename") and OPP-115 chunks (field "source_doc")
  - Backward compatible with your existing 112 chunks
...
class UnifiedRetriever:
    ...
    def _load_chunks(self):
        ...
        docs = set(_get_doc_name(c) for c in self.chunks)
    ...
    def _load_faiss(self):
        ...
        self.index_type = "HNSW" if isinstance(self.index, faiss.IndexHNSWFlat) else "Flat"
        logger.info("Loaded FAISS index: %d vectors (type: %s)", self.index.ntotal, self.index_type)

        if self.index_type == "HNSW":
            self.index.hnsw.efSearch = self.ef_search
            logger.info("HNSW efSearch set to %d", self.ef_search)

        assert self.index.ntotal == len(self.chunks),             f"FAISS desync: {self.index.ntotal} vs {len(self.chunks)} chunks"
    ...
    def dense_search(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        for score, idx in zip(scores, ids):
            ...
            results.append({
                ...
                "text": chunk["text"],
                "score": float(score),
                "source": _get_doc_name(chunk),
                "source_corpus": chunk.get("source_corpus", "unknown"),
                "method": "dense"
            })
    ...
    def verify_indexes(self):
        ...
        logger.info("Index verification: %s (type=%s)", "PASS" if ok else "FAIL", self.index_type)
        return ok
