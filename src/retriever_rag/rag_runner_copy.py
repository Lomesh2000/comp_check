...
def init_local_flan(model_name="Qwen/Qwen2.5-1.5B-Instruct"):
...
Based ONLY on the business text, what is the status of this rule?

Definitions:

SATISFIED = The business text clearly satisfies the rule.

VIOLATED = The business text clearly contradicts the rule.

NOT_MENTIONED = The business text does not provide enough information to determine whether the rule is satisfied.
...
def rerank_rules(G, chunk_embedding, embed_model, top_k=5):
    """
    Rank graph edges (rules) against the business chunk.
    Returns the top-k rule strings.
    """
    predicate_map = {
        "mustProvide": "must provide",
        "mustObtain": "must obtain",
        "mustEnsure": "must ensure",
        "mustImplement": "must implement",
        "mustNotify": "must notify",
        "mustMaintain": "must maintain",
        "mustConduct": "must conduct",
        "mustAppoint": "must appoint",
        "mustBe": "must be",
        "mustBeProcessedFor": "must be processed for",
        "mustBeDeletedAfter": "must be deleted after",
        "hasRightTo": "has the right to",
        "requires": "requires",
        "subjectTo": "is subject to",
        "assists": "assists",
        "manages": "manages",
    }
    rule_texts = []
    ...
    for u, v, data in G.edges(data=True):
        ...
        # rule = f"{u_label} {pred} {v_label}"
        pred_text = predicate_map.get(pred, pred)

        rule = f"{u_label} {pred_text} {v_label}."
    ...
    rule_embs = embed_model.encode(
        rule_texts,
        convert_to_numpy=True
    )

    rule_embs /= (
        np.linalg.norm(rule_embs, axis=1, keepdims=True)
        + 1e-12
    )

    print("\nALL CANDIDATE RULES")

    sims = rule_embs @ chunk_embedding

    order = np.argsort(sims)[::-1]

    print("\nRULE SIMILARITIES")
    for i in order:
        print(f"{sims[i]:.4f} | {rule_texts[i]}")

    return [rule_texts[i] for i in order[:top_k]]
...
def main(args):
    ...
    for idx, c in enumerate(chunks):
        ...
        print("\nTOP RULES AFTER RERANKING")
        for r in top_rules:
            print(r)
        # print("\nNUMBER OF RULES =", len(triples_text.split("\n")))
        print("\nNUMBER OF RULES =", len(top_rules))
        ...
        try:
            ...
            for rule in top_rules:
                ...
                ans = call_llm(
                    prompt,
                    use_openai_priority=not args.prefer_local,
                    model_openai=args.openai_model,
                    max_tokens=32,
                ).strip()
