...
def test_compliance_check(base_url):
    ...
    payload = {
        ...
        "hop_k": 1,
        "max_triples": 60,
        # False by default: this should exercise the same code path the
        # API will use in production (OpenAI). flan-t5-large is NOT faster
        # than the OpenAI API call — it has to load multi-GB weights into
        # memory on first use — and a local-model failure (e.g. no network
        # to download weights, no GPU) would mask whether the API's
        # OpenAI path actually works. Pass --prefer-local explicitly if
        # you want to smoke-test the local fallback specifically.
        "prefer_local": False,
